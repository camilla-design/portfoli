...
function FeedbackCards({ feedbackInfo }: Props) {
  return (
    <article className='flex flex-col rounded-lg items-center space-y-2 flex-shrink-0 w-[400px] md:w-[600px] xl:w-[800px] snap-center bg-[#2272FF]/60 p-5 hover:opacity-100 opacity-70 cursor-pointer transition-opacity duration-200 overflow-hidden'>
        <motion.img
        initial={{
            y: -100,
            opacity: 0,
        ...
        viewport={{
         ...
         <div className='px-0 md:px-5 text-gray-200'>
             ...
             <p className='w-[300px] md:w-[300px] xl:w-[500px] text-sm md:text-base'> {feedbackInfo?.feedback}</p>
         </div>
    </article>
  )
}
